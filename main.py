...
# quantifyfx/ai-signal-engine/main.py
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel
import random
...
class SignalRequest(BaseModel):
    symbol: str = "XAUUSD"
    timeframe: str = "15m"
    telegram_chat_id: str = "" # Optional Telegram Chat ID / Channel Username

@app.get("/")
def home():
    return {"status": "online", "message": "QuantifyFX AI Engine & Telegram Bot Active!"}
...
def generate_signal(req: SignalRequest):
    ...
    signal_payload = {
        ...
        "reasoning": f"ICT 15m Asian Low Liquidity Sweep detected on {req.symbol}. Market structure is aligned with {chosen_direction} bias into 15m Order Block."
    }

    # If Telegram Chat ID or Channel Username is provided, broadcast instantly!
    telegram_status = None
    if req.telegram_chat_id:
        telegram_status = send_telegram_signal(req.telegram_chat_id, signal_payload)

    return {
        "status": "success",
        "signal": signal_payload,
        "telegram_broadcast": telegram_status
    }